...
export const AddSongDialog = () => {
  ...
  const handleSubmit = async () => {
    // Implementation for submitting the new song
    setIsLoading(true);

    try {
      if (!files.audio || !files.image) {
        return toast.error("Both audio and image files are required."); // Handle error: both files are required
      }
      ...
      // Make API call to upload the song
    ...
    <Dialog open={isSongDialogOpen} onOpenChange={setIsSongDialogOpen}>
      ...
      <DialogContent className="bg-zinc-900 border-zinc-700 max-h-[80vh] overflow-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Fill out the form below to add a new song to the library.
          </DialogDescription>
        </DialogHeader>

        <div className="space-y-4 py-4 ">
          {/* Form fields for song details go here */}
          <input
            type="file"
            accept="audio/*"
            ref={audioInputRef}
